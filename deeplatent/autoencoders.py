...
class ImageEncoder(nn.Module):
    ...
    def __init__(
        ...
        for hidden_dim in fc_hidden_dims:
            fc_layers.extend(
                [nn.Linear(prev_dim, hidden_dim), self.activation, self.dropout]
            )
            prev_dim = hidden_dim
    ...
    def forward(
        self, x: torch.Tensor, prevalence_covariates: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
...
class ImageDecoder(nn.Module):
    ...
    def __init__(
        ...
        fc_output_dim = (
            self.first_conv_dim * self.feature_map_size * self.feature_map_size
        )
        ...
        deconv_layers.extend(
            [
                nn.ConvTranspose2d(
                    in_channels, output_channels, kernel_size=4, stride=2, padding=1
                ),
    ...
    def forward(
        self, z: torch.Tensor, content_covariates: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
...
class MADE(nn.Module):
    ...
    def __init__(
        ...
        layer_dims = (
            [dim] + [hidden_dim] * num_hidden + [2 * dim]
        )  # 2*dim for mu and log_sigma
    ...
    def __init__(
        ...
        if use_permutations:
            self.register_buffer(
                "perms", torch.stack([torch.randperm(dim) for _ in range(num_flows)])
            )
        else:
            self.perms = None
...
class MultiModalEncoder(nn.Module):
    def __init__(
        ...
        n_topics: Optional[
            int
        ] = None,  # simplex dim K; if == topic_dim + 1, theta = softmax(z @ V.T)
    ):
        super().__init__()
        ...
        assert moe_type in {
            ...
            "gating",
            "learned_weights",
        }, f"Invalid moe_type: {moe_type}"
        assert poe in {False, "uncorrected", "corrected"}, f"Invalid poe: {poe}"

        self.encoders = nn.ModuleDict(encoders)
        self.topic_dim = topic_dim
        self.n_topics = n_topics if n_topics is not None else topic_dim
        if self.n_topics == topic_dim + 1:
            # (K-1)-dim contrast latent: theta = softmax(z @ V.T) lifts to the K-simplex.
            self.register_buffer("V", contrast_basis(self.n_topics), persistent=False)
        elif self.n_topics != topic_dim:
            raise ValueError(
                f"n_topics must equal topic_dim (identity latent) or topic_dim + 1 "
                f"(contrast latent); got topic_dim={topic_dim}, n_topics={n_topics}."
            )
        self.prior = prior
        self.gating = gating
        ...
        if self.moe_type == "gating" or self.gating:
            if ae_type == "vae":
                if vi_type == "mean_field":
                    ...
                    input_dim = len(encoders) * (
                        self.mixture_components * (2 * topic_dim + 1)
                    )
            else:
    ...
    def product_of_experts_full_rank(
        ...
        if self.poe != "corrected" or self.prior is None:
            ...
            combined_mu = torch.bmm(
                combined_Sigma, combined_precision_mean.unsqueeze(-1)
            ).squeeze(-1)
    ...
    def _mog_unpack(self, z_raw: torch.Tensor):
        ...
        logvars = torch.clamp(
            z_raw[:, C * K : 2 * C * K].view(B, C, K), min=-8.0, max=8.0
        )
    ...
    def latent_to_theta(self, z: torch.Tensor) -> torch.Tensor:
        """Canonical latent -> simplex map.

        For contrast latents (n_topics == topic_dim + 1): softmax(z @ V.T),
        lifting [B, K-1] contrast coordinates to the K-simplex. Otherwise a
        plain softmax over the latent dimensions. The getattr defense keeps
        this valid on encoder objects unpickled from older checkpoints.
        """
        V = getattr(self, "V", None)
        logits = z @ V.t() if V is not None else z
    ...
    def forward(
        ...
        if single_modality is not None:
            ...
            theta = self.latent_to_theta(z_sample)
                ...
                if self.vi_type == "mean_field":
                    ...
                    if self.poe == "corrected" and self.prior is not None:
                        ...
                        if has_full_cov:
                            ...
                            Sigma_p_batch = Sigma_p.unsqueeze(0).expand(B, -1, -1)
                            Lambda_0_diag = 1.0 / (
                                torch.diagonal(Sigma_p_batch, dim1=1, dim2=2) + 1e-8
                            )
                        else:
        ...
        if self.poe and self.ae_type == "vae":
            ...
            if self.vi_type == "mean_field":
                distributions = [
                    (info[0], info[1]) for info in mu_logvar_info if len(info) >= 2
                ]
                if distributions:
                    combined_mu, combined_logvar = self.product_of_experts(
                        distributions, prevalence_covariates=prevalence_covariates
                    )
                ...
                if base_distributions:
                    combined_mu, combined_logvar = self.product_of_experts(
                        base_distributions, prevalence_covariates=prevalence_covariates
                    )
                    ...
                    # Update mu_logvar_info for consistency
                    mu_logvar_info = [
                        (combined_mu, combined_logvar, z0_fused, z_final, log_det_j)
                    ]
                else:
                    # Fallback if no valid distributions
                    z_final = torch.zeros(
                        (
                            prevalence_covariates.size(0)
                            if prevalence_covariates is not None
                            else 1
                        ),
                        self.topic_dim,
                        device=(
                            prevalence_covariates.device
                            if prevalence_covariates is not None
                            else "cpu"
                        ),
                    )
            ...
            if self.vi_type == "mixture_of_gaussians":
                ...
                comp_means = torch.cat(
                    [o[0] for o in modality_outputs], dim=1
                )  # [B, M*C, K]
                comp_logvars = torch.cat(
                    [o[1] for o in modality_outputs], dim=1
                )  # [B, M*C, K]
                ...
                if self.moe_type == "learned_weights":
                    ...
                    gate_in = torch.cat(
                        [
                            torch.cat(
                                (
                                    o[0].reshape(o[0].size(0), -1),
                                    o[1].reshape(o[1].size(0), -1),
                                    o[2],
                                ),
                                dim=1,
                            )
                            for o in modality_outputs
                        ],
                        dim=1,
                    )
                ...
                if w.dim() == 1:
                    ...
                    pis = [
                        o[2] * w[:, m : m + 1] for m, o in enumerate(modality_outputs)
                    ]
                ...
                if self.ae_type == "vae" and self.vi_type == "iaf":
                    ...
                    avg_mu = torch.stack(
                        [info[0] for info in mu_logvar_info], dim=1
                    ).mean(dim=1)
                    avg_logvar = torch.stack(
                        [info[1] for info in mu_logvar_info], dim=1
                    ).mean(dim=1)
                    ...
                    mu_logvar_info = [
                        (avg_mu, avg_logvar, z0_fused, z_final, log_det_j)
                    ]
                else:
                    z_final = torch.stack(modality_outputs, dim=1).mean(dim=1)
                ...
                if self.ae_type == "vae":
                    if self.vi_type == "mean_field":
                        gate_input = torch.cat(
                            [
                                torch.cat((info[0], info[1]), dim=1)
                                for info in mu_logvar_info
                                if len(info) >= 2
                            ],
                            dim=1,
                        )
                        ...
                        gate_input = torch.cat(
                            [
                                torch.cat(
                                    (info[0], info[1].view(info[0].size(0), -1)), dim=1
                                )
                                for info in mu_logvar_info
                                if len(info) >= 2
                            ],
                ...
                if self.ae_type == "vae" and self.vi_type == "iaf":
                    ...
                    fused_mu = torch.sum(
                        torch.stack([info[0] for info in mu_logvar_info], dim=1)
                        * weights,
                        dim=1,
                    )
                    fused_logvar = torch.sum(
                        torch.stack([info[1] for info in mu_logvar_info], dim=1)
                        * weights,
                        dim=1,
                    )
                    ...
                    # Store fused flow information
                    mu_logvar_info = [
                        (fused_mu, fused_logvar, z0_fused, z_final, log_det_j)
                    ]
                else:
        ...
        # Convert to simplex for topic models
        theta = self.latent_to_theta(z_final)
