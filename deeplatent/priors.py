...
class LogisticNormalPrior(Prior):
    ...
    This prior learns a (K-1)-dimensional multivariate Gaussian distribution in
    contrast coordinates, then maps samples to the (K-1)-simplex via a fixed
    orthonormal zero-sum basis V and softmax normalization. Since softmax is
    invariant to shifts along the all-ones logit direction, the (K-1)-dim
    contrast parameterization spans exactly the same logistic-normal family on
    the simplex as a K-dim latent would, without the K unidentified softmax
    gauge directions (mean level and the covariance components involving the
    all-ones direction). The mean can optionally depend on prevalence
    ...
    Mathematical formulation:
        η ~ N(μ(x), Σ)   in R^{K-1}, where μ(x) = W*x (if covariates) or μ (global)
        θ = softmax(V η)  maps to simplex (V = contrast_basis(K), [K, K-1])

    Attributes:
        prevalence_covariate_size: Dimension of prevalence covariates
        n_topics: Number of topics (K)
        n_latent: Latent dimension (K - 1, contrast coordinates)
        V: Fixed orthonormal contrast basis buffer [K, K-1] (not in state dict)
    ...
    def __init__(
        ...
        self.n_latent = n_topics - 1  # contrast coordinates (softmax gauge removed)
        self.learn_cov = learn_cov
        self.register_buffer("V", contrast_basis(n_topics), persistent=False)
        ...
        # Cholesky factor for covariance matrix (K x K).
        ...
        if learn_cov:
            ...
            # Pin Sigma = I by keeping L_flat a fixed zero buffer (not a Parameter, so it is
            # excluded from the optimizer). This removes the logistic-normal logit-scale /
            # temperature degeneracy: scaling logits by s sends (mean, Sigma) -> (s*mean, s^2*Sigma),
            # so a learnable Sigma can collapse and attenuate -- or, under a supervised term,
            # reorganize -- the recovered covariate/outcome coefficients. Use when the prior
            # covariance is known/assumed isotropic (e.g. Sigma_true = I).
            self.register_buffer("L_flat", torch.zeros(tril_count))
    ...
    def get_parameters(self, M_prevalence_covariates):
        ...
        Returns:
            means: Mean vectors [batch_size, K-1] (contrast coordinates)
            covariance: Covariance matrix [K-1, K-1] (shared across batch)
        """
        if self.prevalence_covariate_size > 0:
            ...
            batch_size = (
                M_prevalence_covariates.shape[0]
                if M_prevalence_covariates is not None
                else 1
            )
    ...
    def sample(self, N, M_prevalence_covariates=None, to_simplex=True):
        ...
        if means.shape[0] != N:
            ...
            if self.prevalence_covariate_size == 0:
                ...
                raise ValueError(
                    f"Batch size mismatch: got {means.shape[0]}, expected {N}"
                )

        # Sample from multivariate normal with full covariance (K-1 dimensions)
        dist = MultivariateNormal(
            means, covariance.unsqueeze(0).expand(means.shape[0], -1, -1)
        )
        z_samples = dist.sample()  # Shape: [N, K-1]

        if to_simplex:
            # Lift contrast coordinates to K logits, then softmax to the simplex.
            # getattr defense: prior modules unpickled from pre-0.2.0 checkpoints lack V.
            V = getattr(self, "V", None)
            logits = z_samples @ V.t() if V is not None else z_samples
            z_samples = torch.softmax(logits, dim=1)  # Shape: [N, K]
    ...
    def get_prior_params(self, M_prevalence_covariates, return_full_cov=False):
        ...
        Returns (K-1)-dimensional parameters (contrast coordinates).
    ...
    def get_prevalence_coefficients(self):
        """
        Centered per-topic prevalence coefficients in theta-logit (K) space.

        Lifts the (K-1)-dim contrast-space coefficients of ``mean_net`` through
        the contrast basis: W = V @ mean_net.weight. Columns of V sum to zero,
        so each covariate's coefficients are exactly centered across topics —
        the unique centered representative of the softmax gauge class.

        Returns:
            W: [n_topics, prevalence_covariate_size] detached tensor
            b: [n_topics] detached tensor (lifted bias; zeros if no bias)

        Raises:
            ValueError: If the prior has no prevalence covariates.
        """
        if self.prevalence_covariate_size == 0:
            raise ValueError(
                "get_prevalence_coefficients requires prevalence covariates; "
                "this prior has a global mean only (use global_mean / get_prior_params)."
            )
        W = (self.V @ self.mean_net.weight).detach()
        if self.mean_net.bias is not None:
            b = (self.V @ self.mean_net.bias).detach()
        else:
            b = torch.zeros(self.n_topics, device=W.device)
        return W, b
    ...
    def simulate(self, M_prevalence_covariates, **kwargs):
        ...
        """
        return self.sample(
            M_prevalence_covariates.shape[0], M_prevalence_covariates, to_simplex=True
        )
...
class DirichletPrior(Prior):
    ...
    def get_concentration(self, M_prevalence_covariates):
        ...
        if self.prevalence_covariate_size > 0:
            ...
            concentration = (
                torch.exp(self.log_concentration).unsqueeze(0).expand(batch_size, -1)
            )
    ...
    def sample(self, N, M_prevalence_covariates=None):
        ...
        if concentration.shape[0] != N:
            if self.prevalence_covariate_size == 0:
                concentration = (
                    torch.exp(self.log_concentration).unsqueeze(0).expand(N, -1) + 1e-6
                )
            else:
                raise ValueError(
                    f"Batch size mismatch: got {concentration.shape[0]}, expected {N}"
                )
...
class FixedLogisticNormalPrior(Prior):
    ...
    This prior uses a fixed logistic normal distribution in (K-1)-dim contrast
    coordinates: samples are drawn from a standard multivariate normal N(0, I)
    in R^{K-1} and mapped to the simplex via the fixed orthonormal contrast
    basis V and softmax. Parameters are not learned during training.

    This serves as a non-informative baseline for topic models.

    Mathematical formulation:
        η ~ N(0, I)      in R^{K-1}
        θ = softmax(V η)  (V = contrast_basis(K), [K, K-1])
    ...
    def sample(self, N, M_prevalence_covariates=None, to_simplex=True):
        ...
        Returns:
            Samples [N, K]. If to_simplex=True, samples lie on the simplex.
        """
        # Sample from standard multivariate normal (K-1 contrast dimensions)
...
class FixedDirichletPrior(Prior):
    ...
    def get_prior_params(self, M_prevalence_covariates):
        ...
        concentration = torch.full(
            (batch_size, self.n_topics), self.alpha, device=self.device
        )
